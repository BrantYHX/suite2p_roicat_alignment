...
def main():
    ...
    # Important parameters to be tuned
    dir_allOuterFolders = [r"C:\Users\HAOXUAN YIN\Desktop\code\suite2p data\session_1", r"C:\Users\HAOXUAN YIN\Desktop\code\suite2p data\session_2"]  # suite2p data path
    um_per_pixel = 0.4  # Micrometer per pixel of image field of view
    radius_in = 5   # IMPORTANT PARAMETER: Value in micrometers used to define the maximum shift/offset between two images that are considered to be aligned. Larger means more lenient alignment.
    z_threshold = 8  # IMPORTANT PARAMETER: Z-score required to define two images as aligned. Larger values results in more stringent alignment requirements.
    CLAHE_grid_block_size = 5 # IMPORTANT PARAMETER. Use smaller values for higher precision but higher chance of failure.
    dir_save = r"C:\Users\HAOXUAN YIN\Desktop\code\suite2p data\Notebooks\results"  # save directory
    save_dir_aligned_image = r'C:\Users\HAOXUAN YIN\Desktop\code\suite2p data\Notebooks\results'
    file_name = "aligned_img.npy" 
    save_path = os.path.join(save_dir_aligned_image, file_name) # save directory for the aligned image, used later for visualisation
